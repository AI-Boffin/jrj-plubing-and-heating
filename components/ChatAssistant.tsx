...
const ChatAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: 'Welcome to the JRJ Design Studio. Upload a photo of your space or describe your dream bathroom to see a custom visualization.',
      timestamp: new Date(),
    }
  ]);
  ...
  const handleSendMessage = async (text: string, imageBase64?: string, mimeType?: string) => {
    ...
    const result = await getGeminiResponse(
      text || "Please analyze this image or generate a design vision.",
      imageBase64,
      mimeType
    );
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end">
      ...
      {isOpen && (
        <div className="mb-6 w-[90vw] sm:w-[420px] h-[600px] bg-white rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden border border-slate-100 animate-in fade-in slide-in-from-bottom-10 duration-500">
          {/* Luxury Header */}
          <div className="bg-[#0F172A] p-7 text-white flex justify-between items-center shrink-0">
            <div className="flex items-center space-x-4">
              <div className="w-12 h-12 bg-jrj-teal rounded-2xl flex items-center justify-center shadow-lg shadow-jrj-teal/20">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F8FAFC]">
            {messages.map((msg) => (
              ...
              >
                <div className={`max-w-[88%] rounded-3xl p-5 ${
                  msg.role === 'user' 
          ...
          <div className="p-5 bg-white border-t border-slate-100">
            <div className="flex items-center space-x-3 bg-slate-50 rounded-full px-5 py-2 group focus-within:ring-2 focus-within:ring-jrj-teal/20 focus-within:bg-white transition-all">
              ...
              <button 
                ...
                className={`w-10 h-10 bg-[#0F172A] text-white rounded-full flex items-center justify-center transition-all ${!inputText ? 'opacity-30 scale-90' : 'hover:scale-105 hover:bg-jrj-teal active:scale-95 shadow-lg'}`}
              >
      ...
      <button 
        ...
        className={`group relative w-20 h-20 bg-[#0F172A] text-white rounded-[2rem] shadow-2xl flex items-center justify-center transition-all duration-500 hover:rounded-[1.5rem] active:scale-90 ${isOpen ? 'rotate-[360deg] scale-90' : 'hover:-translate-y-2'}`}
      >
        ...
        {isOpen ? (
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[8px] font-bold tracking-tighter uppercase mt-1 text-white/40">Studio</span>
          </div>
        )}
      </button>
    </div>
